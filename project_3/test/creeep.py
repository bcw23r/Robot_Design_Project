...
def _detect_color(frame, color):
    ...
    result_dict: None 또는 {'x_norm', 'z_mm', 'approx', 'img_pts', 'rvec', 'tvec'}
    """
    ...
    if ratio < BLUE_PAPER_RATIO:
        return None, mask  # 픽셀 비율 미달 → 박스 측면이나 노이즈로 판단, 무시
...
def _camera_thread():
    ...
    while True:
        ...
        if result:
            # 윤곽선 & 꼭짓점
            cv2.drawContours(display, [result['approx']], -1, dot_color, 2)

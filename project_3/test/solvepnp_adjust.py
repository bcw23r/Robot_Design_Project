...
CREEP_DURATION   = 0.6    # 종이 감지 후 저속 전진 시간 (초) — 실측 후 조정
CREEP_SPEED      = 0.35   # 저속 전진 속도 (0~1)
ON_PAPER_Z_MM    = 480.0  # solvePnP Z 이 값 이하면 "종이 위" 판정 (mm) — 실측 후 조정
...
_robot_state = {
    'mode'        : 'SEARCHING',  # 'SEARCHING' | 'CREEPING' | 'ON_PAPER' | 'DONE'
    'color_idx'   : 0,            # 현재 목표 색 인덱스
    'stop_start'  : 0.0,          # ON_PAPER 진입 시각
    'creep_start' : 0.0,          # CREEPING 진입 시각
}
...
def _detect_color(frame, color):
    ...
    result_dict: None 또는 {'x_norm', 'z_mm', 'vis_contour', 'img_pts',
                            'rvec', 'tvec', 'method'}
    rvec/tvec는 solvePnP 실패 시 None (z_mm은 면적 추정값으로 대체).
    """
    ...
    for cnt in cnts:
        ...
        if ok and tvec[2][0] > 0:
            ...
            # solvePnP 실패 → 면적 기반 거리 추정 (조향은 유지, 정지 판정은 부정확)
...
def _camera_thread():
    ...
    while True:
        ...
        result, mask = _detect_color(frame, color)
...
while True:
    ...
    try:
        ...
        # ── [SEARCHING] 종이 위 판정 → CREEPING 전환 ─────────────
        if detected and z_mm < ON_PAPER_Z_MM:
            color_name = COLOR_SEQUENCE[cidx]
            print(f"종이 감지! [{color_name}] Z={z_mm:.0f}mm → 전진 후 정지")
            ser_Ardu.write(f"F 0.00 {CREEP_SPEED:.2f}\n".encode())
            with _state_lock:
        ...
        if best is not None and best['passable'] and abs(best['center']) <= ROT_THRESH:
            ...
            CORNER_REP = 200.0  # 반발 시작 거리 축소 (350→200mm)
            ...
            corner_rep = (crnf_L - crnf_R) * 22.0  # 게인 완화 (45→22)
        ...
        # ── P2: 제자리 회전 (갭 후방 + 근접) ────────────────────
            ...
            _prev_steer = 0.0  # 피벗 회전 전 스무더 리셋

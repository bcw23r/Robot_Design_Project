...
color_lidar_v2.py
─────────────────
color_Lidar_1.py (Primary) + LiDAR_detection_L.py VFH (Secondary) 통합

[호회전 탐색 → VFH 탐색으로 교체]
  기존: SEARCH_TIMEOUT 후 좌우 호회전(arc sweep) 반복
  변경: SEARCH_TIMEOUT 후 VFH 기반 개방 공간 탐색
       LiDAR가 통과 가능한 갭 방향으로 전진하므로
       장애물을 피하면서 새 시야각을 확보해 색상 종이를 자연스럽게 발견

[LiDAR 역할 — 부가 기능 3가지]
  1. 긴급 후진: 전방 150mm 이하 시 즉시 후진 (모든 모드 최우선)
  2. 접근 감속: 색상 접근 중 전방 장애물에 따라 속도 상한 제한
  3. VFH 탐색: 색상 미탐지 시 갭 방향 전진 (호회전 대체)

[색상 감지 — color_Lidar_1.py 원본 그대로]
  ① 강탐지: solvePnP 정밀 접근
  ② 약탐지: 오프셋 기반 저속 유도
  ③ 피크 후 소멸: 종이 위 진입 확인 → STOP

[수정 사항]
  · _extract_quad: 변수명 버그(최고→best) 수정
  · 호회전 로직 제거 → VFH 대체
  · LiDAR 백그라운드 스레드 추가
"""
...
EMERGENCY     = 150.0 # VFH → 긴급 정지 임계 거리 (mm)
...
def main():
    ...
    print(f"  미탐지 탐색: 호회전 → VFH 개방공간 전진")
    print("=" * 65)
            ...
            if weak_cnt is not None:
                ...
                if elapsed < SEARCH_TIMEOUT:
                    ...
                    cv2.putText(vis, f"VFH SEARCH {log}",
                                (5, 58), cv2.FONT_HERSHEY_SIMPLEX,
                                0.50, (100, 200, 255), 2)
                    print(f"  [VFH] {color.upper()} {elapsed:.1f}s 미탐지 → {log}")

...
MAX_POSE_AGE    = 8          # 이전 PnP 결과 재활용 최대 프레임 수 (방법 5)
POSE_DECAY_MM   = 15.0       # 프레임당 전진 거리 추정치 mm (방법 5)
...
# 방법 2: 다중 알고리즘 폴백 체인 (IPPE → SQPNP → ITERATIVE)
...
def _is_valid_quad(pts: np.ndarray) -> bool:
    """방법 4: 추출된 4점이 PnP에 사용 가능한 사각형인지 기하학적으로 검사."""
    if pts.shape != (4, 2):
        return False
...
def _extract_quad_by_angle(contour: np.ndarray) -> np.ndarray:
    """방법 1: 볼록 껍질에서 내각이 가장 작은(= 가장 모서리다운) 4점 직접 선택."""
...
def _extract_quad(contour: np.ndarray) -> np.ndarray:
    ...
    # 방법 4: epsilon 범위 0.40 → 0.50 확대, 4점 발견 시 유효성 검사
...
def solve_paper_pose_with_memory(contour, cam_mat, dist_coeffs):
    """방법 5: PnP 실패 시 이전 유효 결과를 최대 MAX_POSE_AGE 프레임까지 재활용."""
    result = solve_paper_pose(contour, cam_mat, dist_coeffs)
...
def main():
    ...
    while True:
        ...
        if det.get('found'):
            last_seen = time.time()
            cnt       = det['contour']
            pose      = solve_paper_pose_with_memory(cnt, pnp_mat, pnp_dist)
            ...
            if pose is not None:
                ...
                _z_hint        = ls['front_near'] if ls['has_data'] and ls['front_near'] < 2000 else None
                rec_cx, rec_cy = reconstruct_paper_center(
                    _mask_fb, cnt, fw, fh, pnp_mat, _z_hint)
                ...
                log_msg        = f"rect off={offset:+.2f} area={area_r:.2f}"
                cv2.putText(vis, f"RECT A={area_r:.3f} off={offset:+.2f}",
                            (fw//2-80, 38), cv2.FONT_HERSHEY_SIMPLEX,
                            0.65, (200, 255, 200), 2)
                _draw_center(vis, rec_cx, rec_cy, (200, 255, 200))
        ...
        # ② 피크 후 미탐지 (종이 위 진입 중) ───────────────────────
            ...
            if weak_cnt is not None:
                ...
                _z_hint        = ls['front_near'] if ls['has_data'] and ls['front_near'] < 2000 else None
                rec_cx, rec_cy = reconstruct_paper_center(
                    _mask_u, weak_cnt, fw, fh, pnp_mat, _z_hint)

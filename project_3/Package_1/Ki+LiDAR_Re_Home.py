...
NUDGE_SPEED        = 0.40   # 피크 후 미탐지 시 앞으로 밀어 넣는 속도
...
def _compute_vfh(hist, has_pt):
    ...
    lL    = _nearest(hist, has_pt, 270.0, arc_half=50)  # 좌측 횡방향 최근접 거리
    lR    = _nearest(hist, has_pt,  90.0, arc_half=50)  # 우측 횡방향 최근접 거리
                ...
                if elapsed < COLOR_MEMORY_TIME:
                    ...
                    if return_elapsed < OBS_RETURN_TIME:
                        ...
                        if return_speed > 0:
                            ...
                            ser.write(b"S\n")
                        heading_deg += return_steer * STEER_DEG_RATE * dt
                        cv2.putText(vis, f"OBS_RET st={return_steer:+.2f} {return_elapsed:.1f}s",
                                    (5, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 165, 255), 2)
                        print(f"  [OBS_RET] {color.upper()} st={return_steer:+.2f} "
                              f"t={return_elapsed:.1f}/{OBS_RETURN_TIME}s")
                    elif (time.time() - no_obs_since) >= HOME_NO_OBS_TIME \
                            and abs(heading_deg) > 3.0:
                        # 시작점 복귀 모드: 누적 헤딩을 0으로 줄이는 방향으로 조향
                        rh_steer = float(np.clip(
                            -heading_deg * HOME_STEER_KP, -MAX_STEER, MAX_STEER))
                        rh_speed = _speed_limit(SPEED_NEAR)
                        if rh_speed > 0:
                            ser.write(f"F {rh_steer:.2f} {rh_speed:.2f}\n".encode())
                        else:
                            ser.write(b"S\n")
                        heading_deg += rh_steer * STEER_DEG_RATE * dt
                        cv2.putText(vis, f"HOME hdg={heading_deg:+.0f}deg st={rh_steer:+.2f}",
                                    (5, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 255, 128), 2)
                        print(f"  [HOME] {color.upper()} hdg={heading_deg:+.0f}deg st={rh_steer:+.2f}")
                    else:
                        # VFH 탐색 (정지·제자리 회전 없이 전진 위주)
                        log = _vfh_drive(ser)
                        heading_deg   += ls['vfh_steer'] * STEER_DEG_RATE * dt

...
DEBUG = False   # True: 매 프레임 터미널 출력, False: 마일스톤만 출력
...
# 벽-추종 파라미터
WALL_TARGET_DIST   = 250.0  # 유지하려는 장애물 측면 거리 (mm)
WALL_KP            = 0.004  # 거리 오차 → 조향 게인
WALL_MIN_SPAN_DEG  = 30.0   # 벽으로 인식할 최소 각도 폭
WALL_SWITCH_MARGIN = 15.0   # 현재 추종 장애물보다 이만큼 커야 교체 (히스테리시스)
WALL_FOLLOW_SPEED  = 0.45   # 벽-추종 주행 속도

# 나선 탐색 파라미터
SPIRAL_TIMEOUT     = 2.5   # 장애물+색상 미탐지 후 나선 시작까지 대기 (s)
SPIRAL_STEER_INIT  = 0.70  # 나선 초기 조향값 (좁은 원)
SPIRAL_STEER_MIN   = 0.10  # 나선 최대 반경 조향값 (거의 직선)
SPIRAL_EXPAND_TIME = 12.0  # 초기 조향에서 최소 조향까지 걸리는 시간 (s)
SPIRAL_SPEED       = 0.38  # 나선 주행 속도
SPIRAL_DIR         = 1.0   # 나선 방향: 1.0=오른쪽, -1.0=왼쪽
...
_lidar_state = {
    ...
    'vfh_speed'  : 0.65,
    'rot_dir'    : 1.0,
    'dominant'   : None,      # 우세 장애물 클러스터 (벽-추종용)
    'clusters'   : [],        # 전체 장애물 클러스터 목록
}
...
def _cluster_obstacles(hist, has_pt):
    """연속된 LiDAR 포인트를 장애물 클러스터로 그룹화."""
    clusters = []
    i = 0
    while i < N_BINS:
        if has_pt[i] and hist[i] <= DETECT:
            j = i + 1
            while j < N_BINS and has_pt[j] and hist[j] <= DETECT:
                j += 1
            span_deg = (j - i) * BIN_DEG
            min_dist = min(hist[k] for k in range(i, j) if has_pt[k])
            center_cw = ((i + j - 1) / 2.0) * BIN_DEG
            clusters.append({
                'span_deg' : span_deg,
                'min_dist' : min_dist,
                'center_cw': center_cw,
...
def _select_dominant(clusters, prev_dominant):
    """히스테리시스 포함 우세 장애물 선택."""
    if not clusters:
        return None
    largest = max(clusters, key=lambda c: c['span_deg'])
    if largest['span_deg'] < WALL_MIN_SPAN_DEG:
        return None
    if prev_dominant is None:
        return largest
    if largest['span_deg'] > prev_dominant['span_deg'] + WALL_SWITCH_MARGIN:
        return largest   # 더 큰 장애물로 교체
    # 기존 추종 장애물과 가장 가까운 클러스터 유지
    best_match = min(clusters,
                     key=lambda c: abs(c['center_cw'] - prev_dominant['center_cw']))
    if best_match['span_deg'] >= WALL_MIN_SPAN_DEG:
        return best_match
    return largest


def _wall_follow_steer(dominant):
    """우세 장애물 옆면을 따라 주행하는 조향값 계산."""
    center_cw = dominant['center_cw']
    # 부호 있는 각도 (-180~+180): 양수=오른쪽, 음수=왼쪽
    center_signed = center_cw if center_cw <= 180.0 else center_cw - 360.0
    side_sign = 1.0 if center_signed >= 0.0 else -1.0
    dist_err = dominant['min_dist'] - WALL_TARGET_DIST   # 양수=너무 멀다
    return float(np.clip(side_sign * dist_err * WALL_KP, -1.0, 1.0))
    ...
    while True:
        try:
            ...
            if s_flag == 1 and scan_buf:
                ...
                clusters = _cluster_obstacles(hist, has_pt)
                with _lidar_lock:
                    prev_dom = _lidar_state['dominant']
                    new_dom  = _select_dominant(clusters, prev_dom)
                    ...
                    _lidar_state['dominant']   = new_dom
                    _lidar_state['clusters']   = clusters
                scan_buf = []
...
def main():
    ...
    peak_area_r    = 0.0
    no_obs_start   = None   # 장애물+색상 모두 없어진 시각 (나선 타이머)
    spiral_t0      = None   # 나선 시작 시각

    while True:
        cap.grab()                  # 버퍼 오래된 프레임 버리기
        ...
        if det.get('found'):
            ...
            no_obs_start  = None       # 색상 보임 → 나선 타이머 리셋
            spiral_t0     = None
            cnt           = det['contour']
            ...
            if pose is not None:
                ...
                if align_mode:
                    rot_sign = 0.55 if steer > 0 else -0.55 # 제자리 회전 속도 (steer 방향 기반)
                    ser.write(f"T {rot_sign:.2f}\n".encode()) # 제자리 회전 명령 (각도 : steer 방향, 속도: 고정)
                    if DEBUG: print(f"  [ALIGN] {color.upper()} rot={rot_sign:+.0f} steer={steer:+.2f} z={z_mm:.0f}mm")
                    last_seen = time.time()
            ...
            if speed > 0:
                ...
                ser.write(b"S\n")
            if DEBUG: print(f"  [SEEK] {color.upper()} {log_msg} steer={steer:+.2f} spd={speed:.2f}")
        ...
        # ② 피크 후 미탐지 (종이 위 진입 중) ───────────────────────
            ...
            if weak_cnt is not None:
                ...
                if DEBUG: print(f"  [ENTER] {color.upper()} off={weak_offset:+.2f} steer={steer_cmd:+.2f}")
            else:
                on_zone_count += 1
                ...
                if DEBUG: print(f"  [ON] {color.upper()} pk={peak_area_r:.2f} cnt={on_zone_count} nudge={nudge_spd:.2f}")
        ...
        # ③ 미탐지 → VFH 탐색 (호회전 대체) ──────────────────────
            ...
            if weak_cnt is not None:
                ...
                if DEBUG: print(f"  [WEAK] {color.upper()} off={weak_offset:+.2f} steer={steer_cmd:+.2f}")
            else:
                ...
                if elapsed < COLOR_MEMORY_TIME:
                    ...
                    if DEBUG: print(f"  [MEM] {color.upper()} t={elapsed:.2f}s st={steer_cmd:+.2f}")
                else:
                    dom = ls.get('dominant')
                    if dom is not None:
                        # 우세 장애물 추종 (벽-추종)
                        no_obs_start = time.time()   # 장애물 있음 → 나선 타이머 리셋
                        spiral_t0    = None
                        wf_steer = _wall_follow_steer(dom)
                        wf_spd   = _speed_limit(WALL_FOLLOW_SPEED, ls)
                        smoothed_steer = (STEER_SMOOTH_ALPHA * wf_steer
                                          + (1.0 - STEER_SMOOTH_ALPHA) * smoothed_steer)
                        if wf_spd > 0:
                            ser.write(f"F {smoothed_steer:.2f} {wf_spd:.2f}\n".encode())
                        else:
                            ser.write(b"S\n")
                        side_txt = 'R' if dom['center_cw'] <= 180.0 else 'L'
                        cv2.putText(vis,
                                    f"WALL-{side_txt} d={dom['min_dist']:.0f}mm"
                                    f" sp={dom['span_deg']:.0f}° st={smoothed_steer:+.2f}",
                                    (5, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (255, 165, 0), 2)
                        if DEBUG: print(f"  [WALL] {color.upper()} side={side_txt}"
                                        f" dist={dom['min_dist']:.0f}mm"
                                        f" span={dom['span_deg']:.0f}° steer={smoothed_steer:+.2f}")
                    else:
                        # 장애물/색상 모두 없음 → 나선 탐색 또는 VFH
                        if ls.get('clusters'):
                            # 약한 장애물이라도 있으면 타이머 리셋
                            no_obs_start = time.time()
                            spiral_t0    = None
                        else:
                            if no_obs_start is None:
                                no_obs_start = time.time()

                        idle_t = time.time() - no_obs_start if no_obs_start is not None else 0.0
                        if idle_t > SPIRAL_TIMEOUT:
                            # 나선형 탐색
                            if spiral_t0 is None:
                                spiral_t0 = time.time()
                                print(f"  [SPIRAL] 시작 (idle={idle_t:.1f}s)")
                            spiral_elapsed = time.time() - spiral_t0
                            t_ratio  = min(spiral_elapsed / SPIRAL_EXPAND_TIME, 1.0)
                            sp_steer = SPIRAL_DIR * (SPIRAL_STEER_INIT
                                                     - (SPIRAL_STEER_INIT - SPIRAL_STEER_MIN) * t_ratio)
                            sp_steer = float(np.clip(sp_steer, -1.0, 1.0))
                            sp_spd   = _speed_limit(SPIRAL_SPEED, ls)
                            smoothed_steer = (STEER_SMOOTH_ALPHA * sp_steer
                                              + (1.0 - STEER_SMOOTH_ALPHA) * smoothed_steer)
                            if sp_spd > 0:
                                ser.write(f"F {smoothed_steer:.2f} {sp_spd:.2f}\n".encode())
                            else:
                                ser.write(b"S\n")
                            cv2.putText(vis,
                                        f"SPIRAL t={spiral_elapsed:.1f}s st={smoothed_steer:+.2f}",
                                        (5, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (200, 100, 255), 2)
                            if DEBUG: print(f"  [SPIRAL] t={spiral_elapsed:.1f}s"
                                            f" ratio={t_ratio:.2f} steer={smoothed_steer:+.2f}")
                        else:
                            # 아직 타임아웃 전 → VFH 탐색
                            log = _vfh_drive(ser)
                            ...
                            if DEBUG: print(f"  [VFH] {color.upper()} idle={idle_t:.1f}s → {log}")

...
# 360° 회전 탐색 파라미터 (실측 후 조정)
ROTATE_SPEED    = 0.35   # T 명령 회전 속도
ROTATE_360_TIME = 4.5    # 360° 소요 예상 시간 (s)
...
def main():
    ...
    target_idx     = 0
    state          = 'ROTATE_SCAN'
    rotate_start   = time.time()   # 360° 탐색 시작 시각
    on_zone_count  = 0
    stop_start     = None
    last_seen      = time.time()
    last_steer     = 0.0
    ...
    while True:
        ...
        if state == 'STOP':
            ...
            if elapsed >= STOP_DURATION:
                if target_idx < len(TARGETS) - 1:
                    ...
                    last_seen      = time.time()
                    smoothed_steer = 0.0
                    print(f"  ✅ {color.upper()} 완료 → {TARGETS[target_idx].upper()} 360° 탐색 시작")
                else:
                    state = 'DONE'
                    print("  ✅ 전체 미션 완료!")
            continue
        ...
        # ── ROTATE_SCAN (360° 회전 탐색) ──────────────────────────
        if state == 'ROTATE_SCAN':
            det = result.get(color, {})
            found = det.get('found', False)
            if not found:
                found = get_weak_contour(hsv_u, color) is not None

            elapsed_rot = time.time() - rotate_start
            if found:
                state          = 'SEEK'
                last_seen      = time.time()
                smoothed_steer = 0.0
                print(f"  🔍 {color.upper()} 발견 (t={elapsed_rot:.1f}s) → SEEK")
            else:
                ser.write(f"T {ROTATE_SPEED:.2f}\n".encode())
                pct = min(elapsed_rot / ROTATE_360_TIME * 100, 100)
                cv2.putText(vis, f"ROTATE_SCAN {color.upper()} {pct:.0f}%",
                            (fw//2-110, 40), cv2.FONT_HERSHEY_SIMPLEX,
                            0.8, (255, 200, 0), 2)
                if elapsed_rot >= ROTATE_360_TIME:
                    # 360° 완료 후에도 미발견 → 계속 회전 대기 (필요 시 다른 전략으로 교체)
                    rotate_start = time.time()
                    print(f"  🔄 {color.upper()} 360° 완료, 재탐색 중...")
                emg_txt = f" EMG:{ls['emg_near']:.0f}mm" if ls['has_data'] else ""
                cv2.putText(vis, f"ROTATE_SCAN|{color.upper()}{emg_txt}",

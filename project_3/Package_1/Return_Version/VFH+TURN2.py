...
SEARCH_SPIN_TIME  = 3.65    # 1회 360° 스핀 소요 시간(s) — 실측 후 조정
...
def _lidar_worker(ser_l):
    ...
    while True:
        try:
            ...
            print(f"[LIDAR] {e}")
            scan_buf = []
            time.sleep(0.01)   # 시리얼 단절 시 빠른 에러 루프 방지
...
def _extract_quad(contour: np.ndarray) -> np.ndarray:
    ...
    # 병합 폴백: np.delete(O(n²)) 대신 list.pop(O(n))으로 교체
    bl = best.tolist()
    while len(bl) > 4:
        n = len(bl)
        dists = [math.hypot(bl[i][0] - bl[(i+1)%n][0],
                            bl[i][1] - bl[(i+1)%n][1]) for i in range(n)]
        idx = int(np.argmin(dists))
        nxt = (idx + 1) % len(bl)
        bl[idx] = [(bl[idx][0] + bl[nxt][0]) / 2, (bl[idx][1] + bl[nxt][1]) / 2]
        bl.pop(nxt)
    best = np.array(bl, dtype=np.float32)
...
def get_weak_contour(hsv, color: str, mask=None):
    if mask is None:
...
def main():
    ...
    # ── 종료 핸들러 ───────────────────────────────────────────────
    _cleaned = [False]   # atexit + signal 이중 호출 방지
    def _cleanup():
        if _cleaned[0]:
            return
        _cleaned[0] = True
    ...
    while True:
        ...
        # ── STOP (정지 대기) — detector.detect() 불필요, raw 직접 사용 ──
        if state == 'STOP':
            ser.write(b"S\n")
        ...
        # STOP에서 continue했으면 여기 도달하지 않음 → 이후 코드에 ls/result/hsv_u 필요
        ls     = _lidar_read()
        ...
        if det.get('found'):
            ...
            if pose is not None:
                ...
                if align_mode:
                    ...
                    rot_sign = 0.25 if steer > 0 else -0.25
            ...
            # AVOID_ENTER(600mm) 진입 / AVOID_EXIT(720mm) 해제 — VELO_DOWN(400mm)보다 크게
            # 설정하여 충돌 전에 VFH가 먼저 개입하도록 함
            if ls['has_data']:
                if   ls['front_near'] < AVOID_ENTER:  avoiding = True
                elif ls['front_near'] > AVOID_EXIT:   avoiding = False
                if DEBUG and ls['front_near'] < DETECT:
                    print(f"    [LiDAR] front={ls['front_near']:.0f}mm emg={ls['emg_near']:.0f}mm avoiding={avoiding}")
            else:
                avoiding = False
            # 목표 근접 시 AVOID 강제 해제 (z_mm < 300mm에서 VFH가 방향 틀면 종이 지나침)
            if pose is not None and z_mm < DIST_SLOW_MM * 2:
                avoiding = False

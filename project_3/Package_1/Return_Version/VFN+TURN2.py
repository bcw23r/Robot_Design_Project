...
AREA_STOP_THRES = 0.40      # 이 면적 비율 이상이면 종이 위 확정 → 즉시 STOP (PnP 실패 대비)
STEER_GAIN      = 0.015
CONFIRM_FRAMES  = 5
STOP_DURATION   = 1.0
    ...
    while True:
        ...
        if det.get('found'):
            ...
            if pose is not None:
                ...
                # PnP 실패 = 종이가 화면을 꽉 채울 만큼 가까움 → 면적으로 즉시 STOP
                if area_r > AREA_STOP_THRES:
                    state = 'STOP'; stop_start = time.time()
                    ser.write(b"S\n")
                    print(f"  🎯 {color.upper()} 면적 도달! area={area_r:.2f}")
                    with _display_lock:
                        _display_frame[0] = vis
                    continue
                offset  = det['offset']

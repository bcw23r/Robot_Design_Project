...
NUDGE_SPEED        = 0.20   # 피크 후 미탐지 시 앞으로 밀어 넣는 속도
...
OBS_CONFIRM_SEC  = 2.0           # 장애물 소멸 후 추가 장애물 없음을 확인하는 대기 시간 (s)
OBS_RETURN_TIME  = 8.0           # 확인 완료 후 복귀 조향 유지 시간 (s)
...
_lidar_state = {
    ...
    'vfh_speed'  : 0.65,
    'rot_dir'    : 1.0,
    'lat_L'      : 9999.0,   # 좌측(270°±45°) 최근접 거리 — 복귀 방향 판단용
    'lat_R'      : 9999.0,   # 우측(90°±45°) 최근접 거리 — 복귀 방향 판단용
}
...
def _compute_vfh(hist, has_pt):
    """VFH 분석 → (action, steer, speed, rot_dir, emg_near, front_near, lat_L, lat_R)."""
    emg   = _nearest(hist, has_pt, 0.0,   arc_half=80)
    front = _nearest(hist, has_pt, 0.0,   arc_half=40)
    lL    = _nearest(hist, has_pt, 270.0, arc_half=45)  # 좌측 횡방향 최근접 거리
    lR    = _nearest(hist, has_pt,  90.0, arc_half=45)  # 우측 횡방향 최근접 거리
        ...
        # ③ 미탐지 → VFH 탐색 (호회전 대체) ──────────────────────
            ...
            if weak_cnt is not None:
                ...
                if elapsed < COLOR_MEMORY_TIME:
                    ...
                    # 장애물 유무 추적
                    obstacle_now = ls['has_data'] and ls['emg_near'] < OBS_DETECT_DIST
                    if obstacle_now:
                        # 횡방향 비대칭: lL-lR > 0 → 우측 장애물이 더 가까움 → 복귀도 우측
                        lL_now = ls['lat_L']
                        lR_now = ls['lat_R']
                        last_obs_steer = float(np.clip(
                            (lL_now - lR_now) / (DETECT + 1e-9), -1.0, 1.0))
                        if not obs_active:
                            obs_clear_time = None  # 새 장애물 → 복귀 타이머 리셋
                        obs_active = True
                    elif obs_active:
                    ...
                    # obs_clear_time 기준 전체 경과 시간
                    # (None이면 확인+복귀 구간 모두 지난 것으로 처리 → VFH)
                    return_elapsed = (time.time() - obs_clear_time) \
                        if obs_clear_time is not None \
                        else OBS_CONFIRM_SEC + OBS_RETURN_TIME + 1.0

                    if return_elapsed < OBS_CONFIRM_SEC:
                        # ── 확인 대기 구간: 장애물이 다시 나타나는지 2초간 감시 ──
                        # 이 구간에서 obstacle_now==True가 되면 위 블록에서
                        # obs_clear_time=None / obs_active=True 로 리셋됨
                        log = _vfh_drive(ser)
                        smoothed_steer *= (1.0 - STEER_SMOOTH_ALPHA)
                        cv2.putText(vis,
                                    f"OBS_WAIT {return_elapsed:.1f}s / {OBS_CONFIRM_SEC:.0f}s",
                                    (5, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 200, 200), 2)
                        print(f"  [OBS_WAIT] {color.upper()} "
                              f"t={return_elapsed:.1f}s 장애물 재출현 감시 중")
                    elif return_elapsed < OBS_CONFIRM_SEC + OBS_RETURN_TIME:
                        # ── 확인 완료 → 복귀 조향 적용 ─────────────────────────
                        act_t        = return_elapsed - OBS_CONFIRM_SEC
                        return_steer = float(np.clip(
                            last_obs_steer * OBS_RETURN_GAIN, -MAX_STEER, MAX_STEER))
                        return_speed = _speed_limit(SPEED_NEAR)
                        if return_speed > 0:
                            ser.write(f"F {return_steer:.2f} {return_speed:.2f}\n".encode())
                        else:
                            ser.write(b"S\n")
                        cv2.putText(vis,
                                    f"OBS_RET st={return_steer:+.2f} {act_t:.1f}s",
                                    (5, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 165, 255), 2)
                        print(f"  [OBS_RET] {color.upper()} st={return_steer:+.2f} "
                              f"t={act_t:.1f}/{OBS_RETURN_TIME}s")
                    else:
                        # ── 복귀 조향 완료 → VFH 탐색 ───────────────────────────
                        log = _vfh_drive(ser)
                        smoothed_steer *= (1.0 - STEER_SMOOTH_ALPHA)

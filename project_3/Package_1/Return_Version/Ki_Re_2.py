                ...
                if weak_cnt is not None:
                    ...
                    if elapsed < COLOR_MEMORY_TIME:
                        ...
                        if return_elapsed < OBS_CONFIRM_SEC:
                            ...
                            if DEBUG:
                                print(f"  [OBS_WAIT] {color.upper()} t={return_elapsed:.1f}s 장애물 재출현 감시 중")
                            ...
                            if DEBUG:
                                print(f"  [OBS_RET] {color.upper()} st={return_steer:+.2f} t={act_t:.1f}/{OBS_RETURN_TIME}s")
                        else:
                            # ── 복귀 조향 완료 → VFH 탐색 ───────────────────────
                            log = _vfh_drive(ser)
                            smoothed_steer *= (1.0 - STEER_SMOOTH_ALPHA)

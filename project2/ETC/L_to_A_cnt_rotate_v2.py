    ...
    if s_flag == 1 and len(scan_buf) > 15:
        ...
        # ── 우선순위 3: 구석 감지 (B+D) ──────────────────────────────
            ...
            if corner_cnt >= CORNER_THRESH:
                # sector_avg 좌반구(180°~360°)·우반구(0°~180°) 평균 비교 → 열린 쪽으로 회전
                left_half_avg  = sum(sector_avg[4:8]) / 4   # 섹터 4~7 = 180°~360°
                right_half_avg = sum(sector_avg[0:4]) / 4   # 섹터 0~3 = 0°~180°
                pending_rot_dir = +1 if left_half_avg > right_half_avg else -1
                pending_rotate  = True
                extra_back      = CORNER_BACK
                corner_cnt      = 0
                dir_str = "좌회전" if pending_rot_dir > 0 else "우회전"
                print(f"ESCAPE_BD  임계도달 → 후진 {CORNER_BACK}사이클 후 {dir_str} 예정")
                print(f"  좌반구avg={left_half_avg:.0f}mm  우반구avg={right_half_avg:.0f}mm")

...
while True:
    ...
    try:
        ...
        extra_back      = 0
        # 탈출 관련 변수
        MIN_ESCAPE_ANGLE = 30.0    # 최소 회전 각도 (도) ← 여기서 조정
        DEG_PER_CYCLE    = 15.0    # 한 사이클당 회전 각도 (도) ← 로봇 속도에 맞게 조정
        ...
        print(f"  최소회전: {MIN_ESCAPE_ANGLE:.0f}°  /  사이클당: {DEG_PER_CYCLE:.0f}°")
    ...
    if s_flag == 1 and len(scan_buf) > 15:
        ...
        # ── 우선순위 2: 후진 진행 중 ──────────────────────────────────
            ...
            if extra_back == 0 and pending_escape:
                ...
                print(f"ESCAPE_A  후진완료 → 회전 시작  {escape_left}사이클"
                      f"  ({max(MIN_ESCAPE_ANGLE, pending_angle):.0f}°)  steer={escape_steer:+.2f}")
            else:
        ...
        # ── 우선순위 3: 구석 감지 (전방 막힘 + 4개 이상 섹터 막힘) ────
            ...
            print(f"  → 최적섹터={best_idx}({best_center:.0f}°)  avg={best_avg:.0f}mm  "
                  ...
                  f"(최소{MIN_ESCAPE_ANGLE:.0f}°)  예상사이클={int(eff_angle/DEG_PER_CYCLE)}")

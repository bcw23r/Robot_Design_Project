...
GAP_MARGIN   = 10                  # 통과 안전 마진 (mm) — 0→120: 충돌 방지
GAP_MIN_PASS = ROBOT_WIDTH + GAP_MARGIN # 최소 통과 가능 폭: 320mm
GAP_SLOW_W   = GAP_MIN_PASS + 150.0    # 이 폭 미만이면 속도 추가 감속 (470mm)
    ...
    if s_flag == 1:
        if len(scan_buf) > 10:
            ...
            if best is not None and best['passable'] and abs(best['center']) <= ROT_THRESH:
                ...
                # 갭 폭 기반 추가 감속: GAP_MIN_PASS~GAP_SLOW_W 구간에서 0.5→1.0배
                w_ratio = min((best['width'] - GAP_MIN_PASS) / (GAP_SLOW_W - GAP_MIN_PASS), 1.0)
                speed  *= (0.5 + 0.5 * w_ratio)

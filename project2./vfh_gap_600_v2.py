...
GAP_MARGIN   = 05.0              # 통과 안전 마진 (mm)
GAP_MIN_PASS = ROBOT_WIDTH + GAP_MARGIN   # 최소 통과 가능 폭: 230mm
DETECT       = 600.0             # 갭 차단 판단 거리 (mm)
LOOK_AHEAD   = 800.0             # 속도 감속 시작 거리 (mm) — DETECT 너머 장애물도 감속에 반영
...
while True:
    ...
    if s_flag == 1 and len(scan_buf) > 15:
        ...
        if not any(has_pt):
            ...
            # ── P2: 즉각 위험 + 통과 가능 갭 없음 -> 후진 ──
            # 통과 가능한 갭이 존재하면 P3/P4(회전)에서 처리 (후진 불필요)
            elif (emg_near <= EMERGENCY and
                  (best is None or not best['passable'])):
                emg_cnt += 1
                if emg_cnt >= 6:
                    ser_Ardu.write(b"B 0.60\n")
                    extra_back = 2
                    emg_cnt    = 0
                    print("EXTENDED_BACK 시작! (2x) [긴급]")
                else:
                    ser_Ardu.write(b"B 0.60\n")
            ...
            # ── P3: VFH 전진 — 갭이 전방 반구(+-ROT_THRESH) ─
                ...
                # 속도: LOOK_AHEAD 기준 감속 (DETECT 너머 장애물도 반영)
                near_d = nearest_in_arc(hist, has_pt, best['center_cw'], arc_half=20)
                ratio  = min(max((LOOK_AHEAD - near_d) / (LOOK_AHEAD - EMERGENCY), 0.0), 1.0)
                speed  = 0.70 * (1.0 - ratio * 0.55)

                ser_Ardu.write(f"F {steer:.2f} {speed:.2f}\n".encode())
                print(f"VFH_FWD  gap_f= {best['width']:.0f}mm @ {best['center']:+.0f}도  "
            ...
            # T 명령으로 제자리 회전 -> 후진 없이 방향 전환
                ...
                rot_dir = 1.0 if best['center'] > 0 else -1.0 # 회전 방향: +오른쪽 / -왼쪽
                ser_Ardu.write(f"T {rot_dir:.2f}\n".encode())
                print(f"VFH_ROT  gap_b= {best['width']:.0f}mm @ {best['center']:+.0f}도  "
